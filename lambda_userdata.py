...
def create_ec2_instance(template, build_bucket, job_id, sns_arn):
    try:
        userData = '''<script>
        cd %SystemRoot%\\Temp
        echo Start date and time: %DATE% %TIME% > process.log
        aws s3 cp s3://{build_bucket}/build_userdata.bat build.bat >> process.log
        build.bat {build_bucket} {job_id} {sns_arn} >> process.log
        echo End date and time: %DATE% %TIME% >> process.log
        </script>
        <persist>true</persist>'''.format(build_bucket=build_bucket, job_id=job_id, sns_arn=sns_arn)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        instance_info = create_ec2_instance(params['template'], params['bucket'], job_id, params['sns'])
